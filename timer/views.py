...
def submit(request):
    ...
    manager = Profile.objects.get(user=approver)
    manager_email = manager.email
    body = "Time Sheet applied for " + str(start_date) + " : " + str(end_date)

    send_mail("Time Sheet " + str(request.user), body, 'django.training.2019@gmail.com', [manager_email])
    ...
    return redirect("/timer/select_dates/")
