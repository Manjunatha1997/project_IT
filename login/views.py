...
def show_profile(request):
    # request.user will be currenly logged in user's id
    # below query extracts currently logged-in users information
...
def search_profile(request):
    form = SearchForm()
    # request.GET will be dict
    # request.GET = {'user': 1010, }
    if 'user' in request.GET or 'name' in request.GET or 'manager' in request.GET:
        # it may store duplicates
        users = []
        # check if user form field is typed
        if request.GET.get('user'):
            # if you have typed 1010 is user form field
            # profile_info = Profile.objects.filter(user=1010)
...
# d1 = 25
# for user in users:
#    dob = user.dob # x/y/abcd
#    day = x
#    month = y
#    if d1 == day and month == m1
